...
log_filename = os.path.join(output_dir, f"directory_extract_{current_time}.csv")
...
with open('patterns.csv', 'r', encoding='utf-8-sig') as infile, \
    ...
    # Include association fields so we can tell inactive vs not currently used.
    log_writer.writerow([
        'Pattern', 'Status', 'Description', 'Usage', 'Route Partition',
        'Voicemail Profile', 'Active', 'Associated Device Count',
        'Associated Devices', 'Details'
    ]) 
      ...
      <axl:getLine>
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                assoc_device_elements = root.findall('.//{*}associatedDevices/{*}device')
                ...
                associated_devices = [device.text.strip() for device in assoc_device_elements if device.text and device.text.strip()]
                associated_device_count = len(associated_devices)
                associated_devices_text = '; '.join(associated_devices)
                
                print(f"✓ Extracted: {pattern} | Desc: {description}")
                # <-- 4. Write all variables to the log
                log_writer.writerow([
                    pattern, 'Found', description, usage, partition,
                    vm_profile, active, associated_device_count,
                    associated_devices_text, 'Success'
                ]) 
